<script lang="ts">
	...
	type TeacherNavigationItem = {
		href: '/teacher' | '/teacher/materials';
		label: string;
	};

	let { children, user }: { children: Snippet; user: CurrentUser } = $props();

	const navigationItems: TeacherNavigationItem[] = [
		{ href: '/teacher', label: 'Prehled' },
		{ href: '/teacher/materials', label: 'Materialy' }
	];
</script>
	...
	<header class="border-b border-slate-200 bg-white">
		...
		>
			...
			<div class="flex flex-col gap-3 md:flex-row md:items-center">
				<nav aria-label="Ucitelska navigace" class="flex flex-wrap gap-1">
